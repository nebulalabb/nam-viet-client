...
// Cấu trúc cột xuất Excel cho Category
// A STT
// B Tên danh mục
// C Mã danh mục
// D Thuộc danh mục
...
const ExportCategoryView = ({
  ...
  const handleExport = async () => {
    ...
    const table = document.getElementById('exportCategoryTable')

    worksheet.mergeCells('A1:G1')
    worksheet.getCell('A1').value = 'Báo cáo danh sách danh mục'
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} {...props}>
      ...
      <DialogContent className="md:h-auto md:max-w-full z-[100100]" overlayClassName="z-[100099]">
        <DialogHeader>
          <DialogTitle>Danh sách danh mục xuất file</DialogTitle>
        </DialogHeader>

        <div className="max-h-[65vh] overflow-auto md:max-h-[75vh]">
          <div className="overflow-x-auto rounded-lg border">
            <Table className="min-w-full" id="exportCategoryTable">
              <TableHeader>
                <TableRow className="bg-secondary text-xs whitespace-nowrap">
                  <TableHead className="w-8">STT</TableHead>
                  <TableHead className="min-w-40">Tên danh mục</TableHead>
                  <TableHead className="min-w-32">Mã danh mục</TableHead>
                  <TableHead className="min-w-40">Thuộc danh mục</TableHead>
                  <TableHead className="min-w-40">Nhóm bảo hành</TableHead>
              ...
              <TableBody>
                {props?.data?.map((category) => (
                  <TableRow key={category.id}>
                    ...
                    <TableCell>{category.parent?.categoryName || '—'}</TableCell>
                    <TableCell>{category.warrantyPolicy?.name || '—'}</TableCell>
                    <TableCell>
...
export default ExportCategoryView
